...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    const langChanged = prevLangRef.current !== translateLang;
    if (langChanged) {
      setTranslatedCache({});
      prevLangRef.current = translateLang;
    }
    const translateNew = async () => {
      const currentCache = langChanged ? {} : translatedCache;
      const toTranslate = messages.filter((msg) => {
        if (msg.system) return false;
        if (currentCache[msg.id]) return false;
        ...
        >
          ...
          <div className="space-y-2">
            <p className="text-sm font-medium text-foreground">
              Selecione seu humor <span className="text-destructive">*</span>
            </p>
            <div className="flex flex-col items-center gap-2">
            ...
            {!myMood && (
              <p className="text-xs text-destructive text-center">Escolha um humor para continuar</p>
            )}
          </div>

          <Button type="submit" className="w-full active:scale-[0.97]" disabled={!myMood}>
            Entrar na sala
          </Button>
        </form>
      </div>
